...
module.exports = function (grunt) {
  ...
  // Prepare require configs for JS pages
  var files = grunt.file.expand(config.app + '/scripts/pages/*/main.js');
  var requirejsOptions = {};

  files.forEach(function(file) {
    var name = file.split('/').slice(-2).join('/');
    var out = '.tmp/scripts/pages/' + name;
    requirejsOptions[name] = {
      options: {
        name: 'pages/' + name.replace(/(.js)$/, ''),
